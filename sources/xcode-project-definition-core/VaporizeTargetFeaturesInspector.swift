...
public enum VaporizeTargetFeaturesInspector {
  ...
  ) throws -> (name: String, target: XcodeProjectTarget) {
  ...
  private static func declaredBuildConfigurations(in spec: XcodeProjectDefinition) -> [VaporizeBuildConfigurationInspection] {
    (spec.configs ?? [:])
      .map { name, kind in
...
public struct VaporizeTargetFeaturesInspectionReceipt: Codable, Equatable, Sendable {
  ...
  public var schemaRef = VaporizeXcodeProjectReceiptSchema.targetFeaturesInspectionSchemaRef
  ...
  public var boundaries = [
    ...
    "Pkl-equivalent configFiles inspection is represented in XcodeProjectDefinition but fleet-wide app registry inspection is future work.",
  ]
}
