...
| Independent Swift and Xcode selection | Assistants need the default Swift selection and macOS Xcode developer-directory selection to remain explicit and independent. | `toolchain-selection swift -- use ...`; macOS-only `toolchain-selection xcode -- select ...` | CUJ-05 covers embedded Swiftly `use`, macOS-only Xcode provider compilation, `xcode-select` print/switch/reset routing, rejection of lifecycle/inspection/execution behavior, and selection receipt metadata. |
