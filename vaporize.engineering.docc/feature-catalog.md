...
| Contextual package supply | Developers need one temporal dependency request that can consume maintained source or an admitted product without changing logical identity. | Planned Vaporize source/product selection over SwiftPM, Git `pro` records, and contextual resolutions | Requirements-defined in FR-036 through FR-041 and CUJ-28 through CUJ-32 and CUJ-39; resolver, schemas, commands, and tests are pending. See <doc:package-supply-and-build-intelligence>. |
| Provisioned payload and privacy admission | Client-facing products need large-payload delivery without committing oversized artifacts or leaking producer-private information. | Planned Git admission record plus optional depot payload and privacy gate | Requirements-defined in FR-042 and CUJ-33 through CUJ-34; depot adapter, admission service, privacy scanner, and receipts are pending. |
| Source-closure release builds | Product-first development must not be mistaken for reproducible release proof. | Planned full-closure source policy for large release products | Requirements-defined in FR-043 and CUJ-35; implementation and release evidence are pending. |
| Package-supply observability and build intelligence | Operators need state-machine truth and portfolio X-of-Y coverage without putting analytics on the build critical path. | Planned typed lifecycle events, durable local outbox, and asynchronous projector | Requirements-defined in FR-044 through FR-047 and CUJ-36 through CUJ-38; Schema Universal contracts and targetable OS-specific tests are pending. |
