...
| SwiftPM CLI resource-bundle installs | Resource-bearing CLI products need `Bundle.module` resources to survive installation into `~/.swiftpm/bin`. | `install --artifact cli` plus SwiftPM `--show-bin-path` bundle carry | CUJ-22 proves a typed simulation proving ground with processed, copied, decoded JSON, byte-count, and stale-reinstall resource scenarios; see <doc:swiftpm-cli-resource-bundle-installs>. |
...
| Feature-scoped test lifecycle | A major tool needs tests grouped by user journey so each feature can be targeted during development and release review. | CUJ-specific SwiftPM test bundles and `feature-test-lifecycle.md` | CUJ-09 enforces the coverage contract; v0.0.1 has 22 implemented CUJ bundles. |
...
| Release doctor | Release packets drift when docs, CUJs, tests, gates, launch review, provenance, and schema fixtures are updated separately. | `release-doctor --path <package-or-release-root>` | CUJ-17 covers CLI parsing, live-spine pass, release-root resolution, missing-gate failure, and unresolved-root rejection; GATE-33 requires the release-doctor receipt and now enforces CUJ-state coverage evidence. |
