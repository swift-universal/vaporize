...
| Swift documentation toolchain bridge | Assistants need Swift-DocC and Swift execution to follow the owned toolchain route without calling native discovery tools directly. | `toolchain -- swift package generate-documentation`, `swift-docc-cli@swift-universal.clia.sh export`, `swift-docc-preview-cli@swift-universal.clia.sh serve`, `swift-docc-validator-cli@swift-universal.clia.sh workspace`; lower-level fallback: `toolchain -- docc convert ...` | CUJ-05 covers Swift routing, Swift-DocC compiler fallback routing, Swift toolchain DocC resolution, macOS `xcrun` fallback, non-macOS direct tool lookup, and receipt metadata. |
