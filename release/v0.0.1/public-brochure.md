...
| Pkl-backed Xcode project generation first slice | Generates first-slice `.xcodeproj` world-state from an owned Pkl record, including app and tool target semantics. | Proves one Creative Selection v0.2 specimen plus CUJ-14 generated tool fixtures; does not prove fleet parity. |
