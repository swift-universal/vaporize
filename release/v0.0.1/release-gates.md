...
| GATE-06 - Independent toolchain selection owned by Vaporize | EVIDENCE-READY-PENDING-HUMAN-REVIEW | Current host check at 2026-07-19 selected `6.4.x-snapshot-2026-07-16` globally through the installed Vaporize release; the Swift proxy resolves to the canonical installed Vaporize executable, and the independent Xcode provider reports `/Applications/Xcode-27.0.0-beta3.app/Contents/Developer`. CUJ-05 rejects lifecycle, inspection, execution, and the former overloaded `toolchain` spelling. Toolchain acquisition remains a separate provisioning gap. |
| GATE-07 - CLI help reflects release surface | EVIDENCE-READY-PENDING-HUMAN-REVIEW | Vaporize help advertises `use`, `toolchain-selection`, `validate-json`, `inventory`, `cuj-audit`, `--proof-ledger-path`, `--project-ledger-path`, `--project-ledger-csv-path`, `inspect-project-yml`, `inspect-target-features`, `compare-project-yml-pkl`, `import-project-yml`, `generate-project-yml`, `generate-xcodeproj`, `list-targets`, `list-schemes`, `release-doctor`, `--common-process-spec`, `--xcode-product-cache-workspace`, and `--xcode-product-cache-derived-data-path`. |
...
| GATE-11 - README matches release surface | EVIDENCE-READY-PENDING-HUMAN-REVIEW | README names Swift tools 6.4, `use`, `toolchain-selection`, `validate-json`, Apple project migration commands, and shared Xcode workspace product-cache flags. Historical `x-craze-collapse-path` remains documented only as read-only compatibility. |
