...
| FR-020 | Install integrity | An install that reports success must have actually landed the artifact. CLI install fails loud (`ValidationError`) if no executable lands at the install path after `experimental-install`, and prints `verified <product> installed at <path>` on success. App install stages to a same-volume sibling then atomically swaps (`replaceItemAt`/`moveItem`) so an aborted copy never leaves a missing install, fails loud (`InstallerError.installVerificationFailed`) if nothing landed, and leaves no staged residue. Simulated by CUJ-24; enforces the `tooling-silent-fallback-to-wrong-state-not-error-loud` axiom. |
